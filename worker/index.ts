...
type SaleInput = { id?: string; date?: string; kilo?: number; unitPrice?: number; commissionRate?: number };
...
async function upsertSale(request: Request, env: Env, saleId: string) {
  ...
  if (!season) return error("HAL üretim sezonu Kasım-Haziran arasındadır. Temmuz-Ekim dönemine satış kaydı girilemez.", 409);
  ...
  const received = existing ? n(existing.received) : 0;
  ...
  await env.DB.prepare(`INSERT INTO sales (id,date,season,kilo,unit_price,gross,commission_rate,net,received,created_at,updated_at,deleted_at)
    ...
    .bind(saleId, date, season, kilo, unitPrice, gross, commissionRate, net, received, received).run();
